...
export function buildScopedGetter(
  ...
  return [
    ...
    `  if (${mapVar}.has(${contextIdArg})) return ${mapVar}.get(${contextIdArg})!;`,
...
export function buildContextualScopedProxy(
  ...
  return [
    ...
    `const ${createVar} = (__context: ${ctxType}) => ${instantiationExpr};`,
    `const ${mapVar} = new Map${mapType}();`,
    `const ${resolveVar} = () => {`,
    `  const __context = ${contextVar}();`,
    `  if (${mapVar}.has(__context)) return ${mapVar}.get(__context)!;`,
    `  const __instance = ${createVar}(__context);`,
    ...
    `    return Reflect.deleteProperty(${resolveVar}(), __property);`,
    `  },`,
    `  has(_target, __property) {`,
    `    return Reflect.has(${resolveVar}(), __property);`,
    `  },`,
    `  getPrototypeOf() {`,
    `    return Reflect.getPrototypeOf(${resolveVar}());`,
    `  },`,
    `  ownKeys() {`,
    `    return Reflect.ownKeys(${resolveVar}());`,
    `  },`,
    `  getOwnPropertyDescriptor(_target, __property) {`,
    `    const __descriptor = Reflect.getOwnPropertyDescriptor(${resolveVar}(), __property);`,
...
export function generateScopedExpression(match: ScopedMatch, generation: MacroGenerationContext): string {
  ...
  const expression = generation.instantiateContextual(options, "__context");
